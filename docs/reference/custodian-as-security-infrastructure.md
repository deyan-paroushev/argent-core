...
Argent records a signed control lifecycle over an asset: attestation, pledge, drawdown, repayment, release, default, enforcement. Every one of those events is a statement about a specific bar or set of bars. The current chain can order those transparent reference events, reject an identical supplied lot key, enforce release and role rules, and make the synthetic result verifiable. It cannot derive physical identity or know that the bar exists, is allocated to this owner, is really in the vault, or has genuinely been immobilized. The production custodian service supplies canonical identity and deterministic domain-scoped nullification.
...
Argent provides the governed, signed, ordered control projection referencing that custodied metal and the verifiable integrity trail that lets the bank, custodian, and owner reconcile the same authorized state. In the confidential production profile the complete projection remains private; Soroban anchors uniform state-root batches and does not publish the facility graph. Argent does not hold metal, take title, or attest to physical facts. It records and governs attestations made by the party entitled to make them.

The critical dependency, stated plainly: Argent's control record is only as reliable as the custodian attestation it anchors to. A signed private custody transition and its public integrity anchor are trustworthy exactly to the degree that the custodian's confirmation behind them is trustworthy. This is why the custodian relationship is not a detail to arrange later. It is the foundation, including the custodian's canonical asset-identity and deterministic-nullifier service. The custodian must derive the same nullifier for the same bar throughout the governed domain, preserve continuity through key rotation, and actually block release while a pledge is active.

The nullifier does not make the custodian omniscient. It prevents duplicate active allocation only inside the governed Argent/custodian domain. It cannot detect an unregistered paper pledge or activity in another non-participating system. The full specification is [`confidential-control-and-public-integrity.md`](confidential-control-and-public-integrity.md).
